...
void iWQLSODAIntegrator::f( int neq, double t, double *y, double *ydot )
{
	if(neq==0){
		return;
	}
	
	//y is indexed from 1, need a 0-indexed replacement container
	double * y2=new double [neq];
	for(int x=0; x<neq; x++){
		y2[x]=y[x+1];
	}
	model->readVariables(y2,neq);			//send the var data from y2 to the derived class
	model->modelFunction(t);						//FUNCTION CALLED
	model->copyDerivatives(ydot+1,neq);	//get back the derivatives to ydot, but with 1-indexed
...
bool iWQLSODAIntegrator::solve1Step(iWQModel * m, double tstart, double tend, double eps)
{
	//what is this?
	mord[0] = 0;
	mord[1] = 12;
	...
	ntrep=0;
	ixpr=0;
	mesflg=0;		//suppress all messages
	
	model=m;
	
	//printf("LSODA: from %lf to %lf, tol=%e\n",tstart, tend, eps);
	
	if(!m){
		return true;
	}

	int neq = model->mVarLocations.size();
	double rwork1, rwork5, rwork6, rwork7;
	double t, tout;
	int iwork1, iwork2, iwork5, iwork6, iwork7, iwork8, iwork9;
	int itol, itask, istate, iopt, jt, iout, j, k, jdum;

	//alloc dimension dependent storage
	double * atol=new double [neq+1];
	double * rtol=new double [neq+1];
	double * y=new double [neq+1];
	
	//initial state: set in the model before calling the solver on itself
	//m->setInitialValues(yvon);
	
	//get values from the model into the plain storage
	for(k=0; k<neq; k++){
		y[k+1]=(!model->mShouldTakeDelta[k])?*(model->mVarLocations[k]):0.0;
	}
	
	double t0=tstart;
	double tf=tend;
	double dt=tend-tstart;	//this is not hmin, it is the preferred stepsize
	...
	// make that step
	lsoda(neq,y,&t,tout,itol,rtol,atol,itask,&istate,iopt,jt, iwork1, iwork2, iwork5, iwork6, iwork7, iwork8, iwork9, rwork1,rwork5,rwork6,rwork7);
	
	//extract results from the y array into the model
	for(k=0; k<neq; k++){
		*(model->mVarLocations[k])=(model->mShouldTakeDelta[k])?y[k+1]/(tend-tstart):y[k+1];
	}
	
	/*printf("LSODA: t=%lf, dt=%lf", t, dt);
	for(k=0; k<neq; k++){
		printf("\t%lf", y[k+1]);
	}
	printf("\n");*/
	...
	if (istate <= 0){
		//printf("[Error]: LSODA return state = %d\n",istate);
		return false;
	}
...
void iWQLSODAIntegrator::daxpy( int n, double da, double *dx, int incx, double *dy, int incy )
...
double iWQLSODAIntegrator::ddot( int n, double *dx, int incx, double *dy, int incy )
...
void iWQLSODAIntegrator::dgefa( double **a, int n, int *ipvt, int *info )
...
void iWQLSODAIntegrator::dgesl( double **a, int n, int *ipvt, double *b, int job )
...
void iWQLSODAIntegrator::dscal( int n, double da, double *dx, int incx )
...
int iWQLSODAIntegrator::idamax( int n, double *dx, int incx )
...
void iWQLSODAIntegrator::terminate( int *istate )
{
	if ( illin == 5 ) {
...
void iWQLSODAIntegrator::terminate2( double *y, double *t )
{
	//printf("Terminate2 called.\n");
	int i;
...
void iWQLSODAIntegrator::successreturn( double *y, double *t, int itask, int ihit, double tcrit, int *istate )
{
	int i;
...
void iWQLSODAIntegrator::lsoda( int neq, double *y, double *t, double tout, int itol,
	...
	if ( *istate == 1 || *istate == 3 ) {
		...
		/* Optional inputs.   */
			...
			if ( *istate == 1 ) {
				...
				mxordn = iwork8;
				if ( mxordn < 0 ) {
					printf( "lsoda -- mxordn = %d is less than 0", mxordn );
					terminate( istate );
					return;
				}
				if ( mxordn == 0 )
				...
				mxords = iwork9;
				if ( mxords < 0 ) {
					printf( "lsoda -- mxords = %d is less than 0", mxords );
					terminate( istate );
					return;
				}
				if ( mxords == 0 )
...
*/
		...
		if ( yh == NULL ) {
			printf( "lsoda -- insufficient memory for your problem (yh, lenyh=%d)\n", lenyh );
			terminate( istate );
			return;
		}
		for ( i = 1 ; i <= lenyh ; i++ )
		...
		if ( wm == NULL ) {
			free( yh );
			printf( "lsoda -- insufficient memory for your problem (wm, nyh=%d)\n", nyh );
			terminate( istate );
			return;
		}
		for ( i = 1 ; i <= nyh ; i++ )
		...
		if ( ewt == NULL ) {
			free( yh );
			free( wm );
			printf( "lsoda -- insufficient memory for your problem (ewt, nyh=%d)\n", nyh );
			terminate( istate );
			return;
		}
		...
		if ( savf == NULL ) {
			...
			free( wm );
			free( ewt );
			printf( "lsoda -- insufficient memory for your problem (savf, nyh=%d)\n", nyh );
			terminate( istate );
			return;
		}
		...
		if ( acor == NULL ) {
			...
			free( ewt );
			free( savf );
			printf( "lsoda -- insufficient memory for your problem (acor, nyh=%d)\n", nyh );
			terminate( istate );
			return;
		}
		...
		if ( ipvt == NULL ) {
			...
			free( savf );
			free( acor );
			printf( "lsoda -- insufficient memory for your problem (ipvt, nyh=%d)\n", nyh );
			terminate( istate );
			return;
		}
	}
...
*/
		...
		for ( i = 1 ; i <= n ; i++ ) {
			if ( ewt[i] <= 0. ) {
				printf( "lsoda -- ewt[%d] = %g <= 0.", i, ewt[i] );
	...
	while ( 1 ) {
		if ( *istate != 1 || nst != 0 ) {
			if ( ( nst - nslast ) >= mxstep ) {
				//              printf( "lsoda -- %d steps taken before reaching tout", mxstep );
				*istate = -1;
				terminate2( y, t );
				return;
			}
...
*/
	...
	printf( "meth= %d,   order= %d,   nfe= %d,   nje= %d",
		meth, nq, nfe, nje );
	printf( "t= %20.15e,   h= %20.15e,   nst=%d", tn, h, nst );
...
void iWQLSODAIntegrator::stoda( int neq, double *y )
{
...
void iWQLSODAIntegrator::ewset( int itol, double *rtol, double *atol, double *ycur )
{
	int i;
...
void iWQLSODAIntegrator::intdy( double t, int k, double *dky, int *iflag )
/*
...

{
	...
	*iflag = 0;
	if ( k < 0 || k > nq ) {
		printf( "intdy -- k = %d illegal", k );
		*iflag = -1;
		return;
	}
	tp = tn - hu - 100. * ETA * ( tn + hu );
	if ( ( t - tp ) * ( t - tn ) > 0. ) {
		printf( "intdy -- t = %g illegal", t );
...
void iWQLSODAIntegrator::cfode( int meth )
{
	int i, nq, nqm1, nqp1;
...
void iWQLSODAIntegrator::scaleh( double  *rh, double *pdh )
{
	double r;
	int j, i;
	/*
...
void iWQLSODAIntegrator::prja( int neq, double *y )
{
...
double iWQLSODAIntegrator::vmnorm( int n, double *v, double *w )
/*
...
double iWQLSODAIntegrator::fnorm( int n, double **a, double *w )
/*
...
//  double iWQLSODAIntegrator::bnorm()
//  {
//  }   /*   end bnorm   */

void iWQLSODAIntegrator::correction( int neq, double *y, int *corflag, double pnorm,
...
void iWQLSODAIntegrator::corfailure( double *told, double *rh, int *ncf, int *corflag )
{
	int j, i1, i;
...
void iWQLSODAIntegrator::solsy( double *y )
/*
...
void iWQLSODAIntegrator::methodswitch( double dsm, double pnorm, double *pdh, double *rh )
{
...
void iWQLSODAIntegrator::endstoda()
{
	double r;
	int i;
...
void iWQLSODAIntegrator::orderswitch( double *rhup, double dsm, double *pdh, double *rh, int *orderflag )
/*
...
void iWQLSODAIntegrator::resetcoeff()
/*
...
void iWQLSODAIntegrator::freevectors()
{
	int i;
	if(yh){
		for( i=1; i<= m_lenyh ; ++i ){
